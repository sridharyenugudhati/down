...
var SkeletonContainer = require('./SkeletonContainer-8a290dec.js');
...
var UNSAFE_CardGrid_themes_CardGridStyles_css = require('./UNSAFE_CardGrid/themes/CardGridStyles.css.js');
...
var LoadMoreCollection = require('./LoadMoreCollection-7355820a.js');
var Skeleton = require('./Skeleton-691e13a3.js');
...
function CardGridItem({ children, itemKey, itemIndex, isTabbable, isFocused, isFocusRingVisible, isSelected, selectionMode, initialAnimation, setCardDimension }) {
    ...
    const variantClasses = UNSAFE_CardGrid_themes_CardGridStyles_css.multiVariantStyles({
    ...
    const classes = classNames.classNames([ITEM_STYLE_CLASS, UNSAFE_CardGrid_themes_CardGridStyles_css.styles.itemStyle, variantClasses]);
    ...
    const { nodeRef: nodeRefSlide } = useAnimation.useAnimation(initialAnimation || 'none', animationConfigSlide(itemIndex));
    // animation for opacity change
    const { nodeRef: nodeRefOpacity } = useAnimation.useAnimation((initialAnimation && 'opacity') || 'none', animationConfigOpacity(itemIndex));
    const itemRefs = hooks.useMemo(() => utils_PRIVATE_refUtils.mergeRefs(rootRef, nodeRefSlide, nodeRefOpacity), 
    ...
    return (jsxRuntime.jsx("div", { id: hooks_UNSAFE_useId.useId(), ref: itemRefs, class: classes, role: "gridcell", "data-oj-key": itemKey, onKeyDown: handleKeyDown, "aria-selected": selectionMode === 'none' ? undefined : isSelected, ...(typeof itemKey === 'number' && { 'data-oj-key-type': 'number' }), children: jsxRuntime.jsx(hooks_UNSAFE_useTabbableMode.TabbableModeContext.Provider, { value: { isTabbable }, children: jsxRuntime.jsx(FocusTrap.FocusTrap, { isDisabled: !isTabbable, restoreFocusRef: false, children: children }) }) }));
}
...
const MemoizeCardGridItem = compat.memo(CardGridItem, (prev, next) => {
    if (prev && next) {
...
function CardGrid({ children, data, onLoadMore = () => { }, hasMore = false, getRowKey, currentKey, onCurrentKeyChange, selectionMode = 'none', selectedKeys = emptyKeys, onSelectionChange, viewportConfig, 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, initialAnimation = 'slideUp', gutterSize = 'sm' }) {
    ...
    const [colCount, setColCount] = hooks.useState(0);
    const [cardDimension, setCardDimension] = hooks.useState({ width: 0, height: 0 });
    ...
    const handleColCount = hooks.useCallback((rootWidth) => {
    ...
    const [isCurrentTabbableKey, tabbableModeProps] = hooks_PRIVATE_useTabbableModeSet.useTabbableModeSet(cardGridContains, (element) => {
    ...
    /*****************************************************************************************/
    const { currentKeyProps } = hooks_PRIVATE_useCurrentKey.useCurrentKey((element) => utils_PRIVATE_collectionUtils.keyExtractor(element, ITEM_SELECTOR), selectionMode !== 'multiple', 
    ...
    () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, -colCount), () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, colCount), () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, -1), () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, 1), currentKey, onCurrentKeyChange);
    const [showFocusRing, focusRingProps] = hooks_PRIVATE_useCollectionFocusRing.useCollectionFocusRing(cardGridContains, [
        'ArrowUp',
        'ArrowDown',
        'ArrowLeft',
        'ArrowRight'
    ...
    const handleInitialFocus = () => {
        if (!utils_UNSAFE_keys.isKeyDefined(currentKey) &&
            onCurrentKeyChange &&
    ...
    const handleSelectionRangeChange = hooks.useCallback((detail) => {
        if (data && onSelectionChange) {
            const value = utils_PRIVATE_collectionUtils.handleSelectionRange(detail, data, getRowKey);
    ...
    const { selectionProps } = hooks_PRIVATE_useSelection.useSelection((element) => {
        ...
        return utils_PRIVATE_collectionUtils.keyExtractor(element, ITEM_SELECTOR);
    }, selectedKeys, selectionMode, false, 'replace', false, handleSelectionChange, anchorKey.current, currentKey, (currentKey, isPrev) => () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, isPrev ? -colCount : colCount), (currentKey, isPrev) => () => utils_PRIVATE_collectionUtils.getPrevNextKeyByCount(data, getRowKey, currentKey, isPrev ? -1 : 1), (key) => {
        if (rootRef.current) {
    ...
    const getItemContext = hooks.useCallback((context) => {
        ...
        const selectorRenderer = selectionMode === 'multiple'
            ? () => (jsxRuntime.jsx(UNSAFE_Selector.Selector, { onChange: onSelectionChange, rowKey: key, selectedKeys: selectedKeys }))
            : undefined;
        return {
            index: context.index,
            data: context.data,
            ...
            isSelected: utils_UNSAFE_keys.containsKey(selectedKeys, key)
        };
    ...
    const childrenComponent = (context) => {
        ...
        const cardGridFunc = (context) => (jsxRuntime.jsx(MemoizeCardGridItem, { itemKey: cardItemContext.metadata.key, itemIndex: cardItemContext.index, itemData: cardItemContext.data, isTabbable: isTabbable, isFocused: isFocused, isFocusRingVisible: isFocusRingVisible, isSelected: isSelected, selectionMode: selectionMode, ...(isInitialAnimation.current && { initialAnimation }), ...(context.index === 0 && { setCardDimension }), children: children(context) }, cardItemContext.metadata.key));
    ...
    viewportConfig = utils_PRIVATE_collectionUtils.getViewportConfig(rootRef, viewportConfig);
    const collectionComponent = (jsxRuntime.jsx("div", { role: "row", children: jsxRuntime.jsx(UNSAFE_Grid.Grid, { ...gridProps, children: jsxRuntime.jsx(LoadMoreCollection.LoadMoreCollection, { data: data, hasMore: hasMore, onLoadMore: onLoadMore, loadMoreIndicator: skeletons, loadMoreThreshold: gutterSizePX + 4, viewportConfig: viewportConfig, children: childrenComponent }) }) }));
    return (jsxRuntime.jsx("div", { ...utils_UNSAFE_mergeProps.mergeProps(tabbableModeProps, currentKeyProps, focusRingProps, selectionProps, {
            onFocus: handleInitialFocus,
            onPointerDown: handlePointerDown
        }), ref: rootRef, role: "grid", class: UNSAFE_CardGrid_themes_CardGridStyles_css.styles.baseStyle, tabIndex: 0, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, "aria-multiselectable": ariaMultiSelectable, "aria-rowcount": ariaRowcount, "aria-colcount": colCount, children: data == null ? defaultLoadingIndicator(gridProps) : collectionComponent }));
}
...
jsxRuntime.jsx(SkeletonContainer.SkeletonContainer, { minimumCount: 1, children: () => {
        return jsxRuntime.jsx(Skeleton.Skeleton, { height: `calc(${height || DEFAULT_SKELETON_HEIGHT}px)` });
    } }, isUnique && hooks_UNSAFE_useId.useId() + new Date().getTime())));
/**
 * The default loading indicator will display 25 card skeletons
 * @param gridProps the Grid props
 */
const defaultLoadingIndicator = (gridProps) => (jsxRuntime.jsx(UNSAFE_Grid.Grid, { ...gridProps, children: getSkeletonsByCount(DEFAULT_SKELETON_NUMBER) }));
/**
...
const findDefaultLoadMoreIndicator = (gridProps, totalCount, colCount, skeletonHeight) => {
    ...
    const skeletonRows = skeletons.map((s, i) => (jsxRuntime.jsx("div", { ...(i == 0 && { class: LoadMoreCollection.LOADMORE_STYLE_CLASS }), style: { gridColumn: `span ${gridColumns[i]}` }, children: jsxRuntime.jsx(UNSAFE_Grid.Grid, { ...skeletonGridProps, children: s }) })));
    return skeletonRows;
};
...
//# sourceMappingURL=CardGrid-1a25f7df.js.map

...
var ColorUtils = require('./ColorUtils-ced651e7.js');
var LayoutUtils = require('./LayoutUtils-1a4befaa.js');
...
var PRIVATE_TrackResizeContainer = require('./PRIVATE_TrackResizeContainer.js');
...
var hooks_PRIVATE_useTextDimensions = require('./hooks/PRIVATE_useTextDimensions.js');
var hooks_PRIVATE_useVizSelection = require('./hooks/PRIVATE_useVizSelection.js');
...
require('./hooks/PRIVATE_useSize.js');
require('./hooks/PRIVATE_useResizeObserver.js');
...
const useTagCloudDatatip = ({ text, rootRef, focusedItemRef, focusedItemInfo }) => {
    const { direction } = hooks_UNSAFE_useUser.useUser();
    const isRtl = direction === 'rtl';
    const [offset, setOffset] = hooks.useState(calculateOffset(rootRef, focusedItemInfo, isRtl, focusedItemRef));
...
function TagCloud({ selectionMode = 'none', layout = 'cloud', height = '112x', width = '100%', items, onItemAction, onItemInput, ...props }) {
    ...
    const selectionProps = hooks_PRIVATE_useVizSelection.useSelection({
        idExtracter,
        selection: props.selectedIds,
        selectionMode: selectionMode,
        onChange: props.onSelectionChange
    });
    const mergedProps = utils_UNSAFE_mergeProps.mergeProps(eventsProps, datatipProps, selectionProps);
    const { direction } = hooks_UNSAFE_useUser.useUser();
    return (jsxRuntime.jsx(PRIVATE_TrackResizeContainer.TrackResizeContainer, { width: utils_UNSAFE_size.sizeToCSS(width) || width || '0px', height: utils_UNSAFE_size.sizeToCSS(height) || height || '0px', children: (width, height) => {
            const { textMeasureContent, getTextDimensions } = hooks_PRIVATE_useTextDimensions.useTextDimensions();
            const [layoutTags, setLayoutTags] = hooks.useState([]);
            hooks.useLayoutEffect(() => {
            ...
            return (jsxRuntime.jsxs("div", { ref: rootRef, tabIndex: isReadOnly ? undefined : 0, style: { width, height }, class: UNSAFE_TagCloud_themes_TagCloudStyles_css.baseStyles, ...ariaProps, ...mergedProps, children: [layoutTags.map((item, itemIndex) => {

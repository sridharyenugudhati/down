...
var hooks_PRIVATE_useFloating = require('./index-989087dc.js');
...
var utils = require('./utils-302a51a4.js');
...
var vanillaExtractDynamic_esm = require('./vanilla-extract-dynamic.esm-38df8d31.js');
...
const FloatingTail = compat.forwardRef(({ data, anchorRef, tail, isRtl, backgroundColor }, ref) => {
    const { styles, baseTheme } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_Floating_themes_redwood_FloatingTheme.FloatingRedwoodTheme);
    ...
    const stableRef = hooks.useMemo(() => utils_PRIVATE_refUtils.mergeRefs(ref, arrowRef), [ref, arrowRef]);
    return (tail === 'simple' && (jsxRuntime.jsxs("div", { ref: stableRef, class: classNames.classNames([baseTheme, baseStyle]), style: arrowStyles.arrowStyle, children: [' ', jsxRuntime.jsx("div", { class: classNames.classNames([baseStyle]), style: arrowStyles.arrowInnerStyle }), ' '] })));
});
...
const Floating = compat.forwardRef(({ children, backgroundColor, onClickOutside, placement = 'bottom', anchorRef, offsetValue, class: className, tail = 'none', flipOptions = { mainAxis: true, crossAxis: false }, shiftOptions = { mainAxis: true, crossAxis: false }, sizeOptions = { isMaxHeightAdjusted: false, isMaxWidthAdjusted: false }, boundaryOptions = { padding: 0 }, onPosition }, ref) => {
    ...
    const middleware = [
        hooks_PRIVATE_useFloating.offset(offsetValue),
        hooks_PRIVATE_useFloating.b({
            mainAxis: flipOptions.mainAxis,
            crossAxis: flipOptions.crossAxis,
            fallbackPlacements: flipOptions.fallbackPlacements?.map((placement) => hooks_PRIVATE_useFloating.logicalSide(placement, direction)),
            padding: boundaryOptions.padding
        }),
        hooks_PRIVATE_useFloating.D({
            mainAxis: shiftOptions.mainAxis,
            crossAxis: shiftOptions.crossAxis,
            limiter: hooks_PRIVATE_useFloating.L({
                mainAxis: shiftOptions.mainAxis,
                crossAxis: shiftOptions.crossAxis
            })
        }),
    ...
    if (sizeOptions.isMaxHeightAdjusted || sizeOptions.isMaxWidthAdjusted) {
        middleware.push(hooks_PRIVATE_useFloating.k({
    ...
    const { x, y, reference, floating, refs, update } = hooks_PRIVATE_useFloating.useFloating({
        placement: placement,
    ...
    hooks.useEffect(() => {
        ...
        return hooks_PRIVATE_useFloating.N(refs.reference.current, refs.floating.current, update);
    ...
    const floatingClasses = classNames.classNames([className, UNSAFE_Floating_themes_FloatingStyles_css.floatingStyle, visibilityStyles]);
    const content = (jsxRuntime.jsxs("div", { class: floatingClasses, ref: stableRef, style: { ...positionStyle, ...assignedVars }, children: [children, jsxRuntime.jsx(FloatingTail, { ref: arrowRef, backgroundColor: backgroundColor, data: positionData, anchorRef: anchorRef, tail: tail, isRtl: isRtl })] }));
    return content;
});

exports.Floating = Floating;
//# sourceMappingURL=Floating-ac3c61ed.js.map

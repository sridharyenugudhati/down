...
var UNSAFE_SvgShapes_themes_SvgShapesStyles_css = require('./UNSAFE_SvgShapes/themes/SvgShapesStyles.css.js');
var classNames = require('./classNames-9be98b10.js');


const Ellipse = ({ cx, cy, rx, ry, scaleX, scaleY, tx, ty, fill, stroke }) => {
    return (jsxRuntime.jsx("ellipse", { cx: cx + tx, cy: cy + ty, rx: rx * scaleX, ry: ry * scaleY, fill: fill, stroke: stroke, strokeWidth: 1, vectorEffect: "non-scaling-stroke" }));
};
...
const Path = ({ scaleX, scaleY, tx, ty, fill, stroke }) => {
    return (jsxRuntime.jsx("path", { fill: fill, stroke: stroke, strokeWidth: 1, vectorEffect: "non-scaling-stroke", transform: `translate(${tx} ${ty}) scale(${scaleX} ${scaleY})`, d: HUMAN_PATH_CMDS }));
};
...
const Polygon = ({ type, scaleX, scaleY, tx, ty, height, width, fill, stroke }) => {
    ...
    return (jsxRuntime.jsx("polygon", { fill: fill, stroke: stroke, strokeWidth: 1, vectorEffect: "non-scaling-stroke", transform: `translate(${tx} ${ty}) scale(${scaleX} ${scaleY})`, points: cmds.join(' ') }));
};

const Rect = ({ x, y, width, height, scaleX, scaleY, tx, ty, fill, stroke }) => {
    return (jsxRuntime.jsx("rect", { x: x + tx, y: y + ty, width: width * scaleX, height: height * scaleY, fill: fill, stroke: stroke, strokeWidth: 2, vectorEffect: "non-scaling-stroke" }));
};
...
const Marker = ({ scale, type, height = 100, width = 100, fill, stroke, preserveRectAspectRatio }) => {
    ...
    switch (type) {
        case 'human': {
            return (jsxRuntime.jsx(Path, { type: "human", tx: tx, ty: ty, fill: fill, stroke: stroke, width: width, height: height, scaleX: scaleX, scaleY: scaleY }));
        }
        case 'rectangle':
        case 'square': {
            return (jsxRuntime.jsx(Rect, { x: 0, y: 0, width: width, height: height, scaleX: scaleX, scaleY: scaleY, fill: fill, stroke: stroke, tx: tx, ty: ty }));
        }
        case 'circle':
        case 'ellipse': {
            return (jsxRuntime.jsx(Ellipse, { cx: width / 2, cy: height / 2, scaleX: scaleX, scaleY: scaleY, fill: fill, stroke: stroke, rx: width / 2, ry: ry, tx: 0, ty: 0 }));
        }
        default: {
            return (jsxRuntime.jsx(Polygon, { fill: fill, stroke: stroke, type: type, scaleX: scaleX, scaleY: scale, tx: tx, ty: ty, height: height, width: width }));
        }
    }
};
...
const Line = ({ lineWidth = 3, lineStyle = 'solid', color, lineLength }) => {
    ...
    const { lineBaseStyle, lineDottedStyle, lineDashedStyle } = UNSAFE_SvgShapes_themes_SvgShapesStyles_css.styles;
...
const SvgSymbol = ({ markerShape, lineStyle = 'none', lineLength, lineWidth, lineColor, borderColor, markerColor, setAspectRatioNone }) => {
    ...
    return (jsxRuntime.jsxs("svg", { width: "100%", height: "100%", viewBox: "0 0 100 100", class: UNSAFE_SvgShapes_themes_SvgShapesStyles_css.styles.svgSymbolBaseStyle, preserveAspectRatio: setAspectRatioNone ? 'none' : undefined, children: [lineStyle != 'none' && (jsxRuntime.jsx(Line, { lineStyle: lineStyle, lineWidth: lineWidth, color: lineColor, lineLength: lineLength })), markerShape != 'none' && (jsxRuntime.jsx(Marker, { type: markerShape, scale: markerScale, height: 100, width: 100, preserveRectAspectRatio: !setAspectRatioNone, fill: markerColor, stroke: borderColor }))] }));
};

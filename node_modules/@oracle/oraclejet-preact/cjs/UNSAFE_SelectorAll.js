 ...
 */
function SelectorAll({ selected, onChange }) {
    const pressHandler = (event) => {
        if (selected == 'none') {
            onChange && onChange({ value: { all: true, deletedKeys: new Set() }, target: event.target });
        }
        else {
            onChange && onChange({ value: { all: false, keys: new Set() }, target: event.target });
        }
    };
    ...
    return (jsxRuntime.jsx("div", { class: UNSAFE_Selector_themes_SelectorStyles_css.styles.container, ...pressProps.pressProps, children: jsxRuntime.jsx("div", { class: UNSAFE_Selector_themes_SelectorStyles_css.styles.base, children: jsxRuntime.jsx(Flex.Flex, { align: "center", justify: "center", width: "11x", height: "11x", children: jsxRuntime.jsx(UNSAFE_Selector.Checkbox, { checked: selected !== 'none', isPartial: selected === 'partial' }) }) }) }));
}

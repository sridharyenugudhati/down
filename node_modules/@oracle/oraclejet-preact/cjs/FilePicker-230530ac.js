...
var stringUtils = require('./stringUtils-16e85428.js');
...
var clientHints = require('./clientHints-8eb1c993.js');
...
const getDefaultTriggerContent = (isDisabled, primaryText, secondaryText, translations, selectionMode) => {
    const { styles: { disabledPrimaryTextStyle, primaryTextStyle, disabledSecondaryTextStyle, secondaryTextStyle } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_FilePicker_themes_redwood_FilePickerTheme.FilePickerRedwoodTheme);
    return [
        jsxRuntime.jsx("div", { class: isDisabled ? disabledPrimaryTextStyle : primaryTextStyle, children: getPrimaryText(primaryText, translations) }),
        jsxRuntime.jsx("div", { class: isDisabled ? disabledSecondaryTextStyle : secondaryTextStyle, children: getSecondaryText(secondaryText, translations, selectionMode) })
    ];
};
...
const DefaultFilePicker = (props) => {
    const { interactionProps, applyActiveStyle, applyHoverStyle, applyPseudoHoverStyle } = hooks_UNSAFE_useInteractionStyle.useInteractionStyle(false);
    const { classes, styles: { maxWidth, inner, activeStyle, innerNeedsEventsHoverStyle, innerPseudoHoverStyle, invalidDrop, validDrop } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_FilePicker_themes_redwood_FilePickerTheme.FilePickerRedwoodTheme, {
    ...
    const mergedEventProps = utils_UNSAFE_mergeProps.mergeProps(pressProps, interactionProps);
    ...
    return (jsxRuntime.jsx("div", { ...mergedEventProps, class: wrapperClasses, tabIndex: isTabbable ? 0 : -1, role: "button", "aria-label": getAriaLabel({
            ...
            secondaryText,
            selectionMode
        }), ref: rootRef, style: getStyles(width, height), children: jsxRuntime.jsx("div", { class: classNames.classNames([inner, validityClass, ...innerInteractiveClasses]), ...dragAndDropProps, children: getDefaultTriggerContent(false, primaryText, secondaryText, translations, selectionMode) }) }));
};
/**
...
const DisabledFilePicker = ({ children, primaryText, secondaryText, translations, selectionMode, width, height, ...props }) => {
    const { classes, styles: { maxWidth, inner, noChildrenMargin } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_FilePicker_themes_redwood_FilePickerTheme.FilePickerRedwoodTheme, {
...
const CustomFilePicker = (props) => {
    const { interactionProps, applyHoverStyle, applyPseudoHoverStyle } = hooks_UNSAFE_useInteractionStyle.useInteractionStyle(false);
    const { classes, styles: { maxWidth, noChildrenMargin } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_FilePicker_themes_redwood_FilePickerTheme.FilePickerRedwoodTheme, {
        hoveredState: applyHoverStyle ? 'isHover' : undefined,
        pseudoHoveredState: applyPseudoHoverStyle ? 'isPseudoHover' : undefined
    });
    const rootRef = hooks.useRef(null);
    const { validation, dragAndDropProps, pressProps } = useFilepickerElementHelper(props, rootRef);
    const mergedEventProps = utils_UNSAFE_mergeProps.mergeProps(pressProps, dragAndDropProps, interactionProps);
    // These are backdoor handles that are used for testing purposes only
    useSetupTestRef(dragAndDropProps, validation, props.testHandlersRef || null);
...
//# sourceMappingURL=FilePicker-230530ac.js.map

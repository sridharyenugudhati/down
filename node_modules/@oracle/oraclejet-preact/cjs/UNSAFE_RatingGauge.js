...
const RatingStar = ({ isSelected, isDisabled, isReadOnly, color = 'neutral' }) => {
    ...
    const { variantClasses, styles: { ratingStarBaseStyle, innerRatingStarColor, outerRatingStarColor, outerDisabledHighContrastBaseStyle, outerDisabledHighContrastSelectedStyle, outerDisabledHighContrastUnselectedStyle } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_RatingGauge_themes_redwood_RatingGaugeTheme.RatingGaugeRedwoodTheme, {
        color: enumColor,
...
const RatingGaugeItem = ({ fillRatio, isDisabled, isReadonly, color }) => {
    ...
    const { styles: { fractionalStarBaseStyle } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_RatingGauge_themes_redwood_RatingGaugeTheme.RatingGaugeRedwoodTheme);
...
const useEvents = (max, step, value, rootRef, dimensionsRef, isInteractive, onCommit, onInput) => {
    ...
    const keyboardEventsProps = hooks_PRIVATE_useDvtMeterEvents.useKeyboardEvents(value, 0, max, step, currentInputRef, onCommit, onInput);
    return {
...
function useTooltipOrDatatip({ max, value, isReadonly, isDisabled, tooltip, datatip, thresholds, ariaDescribedBy, width }) {
    ...
    if (!text && thresholds && !isDisabled) {
        text = utils_PRIVATE_meterUtils.findThreshold(value, thresholds)?.accessibleLabel;
        disabled = false;
    }
...
function RatingGauge({ max = 5, value = 0, size, color = 'neutral', step = 1, isReadonly, isDisabled, ...props }) {
    utils_PRIVATE_meterUtils.validateRange(0, max, value, step);
    ...
    const ariaProps = utils_PRIVATE_meterUtils.getMeterAriaProps(value, 0, max, props['aria-label'], props['aria-labelledby'], props.thresholds, isDisabled, isReadonly, props.tooltip);
    const mergedEventProps = utils_UNSAFE_mergeProps.mergeProps(eventProps, tooltipProps);
    const itemColor = utils_PRIVATE_meterUtils.getThresholdColorFromValue(value, color, props.thresholds);
    const { isTabbable } = hooks_UNSAFE_useTabbableMode.useTabbableMode();
    const { classes, styles: { baseStyle, interactiveStyle, itemStyle, sizeStyle } } = hooks_UNSAFE_useComponentTheme.useComponentTheme(UNSAFE_RatingGauge_themes_redwood_RatingGaugeTheme.RatingGaugeRedwoodTheme, {
        size
    });
    return (jsxRuntime.jsxs(preact.Fragment, { children: [jsxRuntime.jsx("div", { ...ariaProps, ref: rootRef, class: classNames.classNames([classes, baseStyle, isInteractive ? interactiveStyle : undefined]), tabIndex: isTabbable && !isDisabled ? 0 : -1, ...mergedEventProps, children: [...Array(max)].map((_, index) => {

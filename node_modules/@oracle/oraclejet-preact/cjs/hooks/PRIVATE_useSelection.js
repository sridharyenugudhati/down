...
function useSelection(keyExtractor, selectedKeys, selectionMode, isSelectionRequired, selectionBehavior, allowEnterKey, onChange, anchorKey, currentKey, getVerticalPrevNextKey, getHorizontalPrevNextKey, scrollToKey, onRangeChange) {
    ...
    const onClick = hooks.useCallback((event) => {
        handleSelect(event, keyExtractor, selectedKeys, selectionMode, utils_PRIVATE_collectionUtils.isMobile() ? 'toggle' : selectionBehavior, isSelectionRequired, keyboardSelectionCursor, anchorKey, onChange, onRangeChange);
        if (!event.shiftKey) {

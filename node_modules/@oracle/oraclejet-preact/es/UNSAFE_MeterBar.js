...
function MeterBar({ max = 100, min = 0, value = 0, step = 1, length = '100%', size, orientation = 'horizontal', isTrackRendered = true, thresholdDisplay = 'all', indicatorSize = 1, ...props }) {
    ...
    const pointerEventsProps = usePointerEvents(value, getValueFromEvent, barTrackRef, currentInputRef, props.onCommit, props.onInput);
    ...
    return (jsxs(Fragment, { children: [jsx("div", { tabIndex: !isTabbable ? -1 : 0, ...mergedProps, style: {
                    width: isHoriz ? sizeToCSS(length) : undefined,
                    height: isHoriz ? undefined : sizeToCSS(length)
                }, class: classNames([classes]), children: jsxs(BarTrack, { ref: barTrackRef, orientation: orientation, color: !isTrackRendered || (thresholdDisplay === 'all' && props.thresholds)
                        ? 'transparent'
                        ...
                        ...getMarginClass(sizeStyle, isHoriz, barTrackMarginTopStyle, barTrackMarginStartStyle, barTrackMarginBottomStyle, barTrackMarginEndStyle)
                            ...
                            ]), color: indicatorColor }), props.referenceLines?.map((item) => (jsx(ReferenceLineBar, { color: item.color, orientation: orientation, percentage: `${((item.value - min) * 100) / (max - min)}%`, position: item.position })))] }) }), datatipContent] }));
}

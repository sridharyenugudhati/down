...
import { InlineUserAssistanceContainer, InlineHelp, InlineUserAssistance } from './UNSAFE_UserAssistance.js';
import { useMessageSeverity } from './hooks/PRIVATE_useMessageSeverity.js';
import { useAccessibleContext } from './hooks/UNSAFE_useAccessibleContext.js';
import './utils/UNSAFE_stringUtils.js';
import { m as merge } from './stringUtils-0805904b.js';
import './UNSAFE_Theme.js';
import './CheckboxRadioFieldStyles.styles.css';
import { LabelValueLayout } from './UNSAFE_LabelValueLayout.js';
import { C as CheckboxRadioContext } from './CheckboxRadioContext-172f4c19.js';
import { F as Flex } from './Flex-1ec75561.js';
import { x as xUnits } from './utils-79fd9e1a.js';
...
import './hooks/UNSAFE_useFormFieldContext.js';
 ...
 * @ignore
 */
function useCheckboxRadioField({ ariaDescribedBy, baseId, hasValue, hasTopUserAssistance, hasBottomUserAssistance, isReadonly, isRequired = false, label, labelEdge, messages }) {
...
const CheckboxRadioField = forwardRef(({ 'aria-describedby': ariaDescribedBy, assistiveText, baseId, direction = 'column', hasValue, helpSourceLink, helpSourceText, isDisabled, isReadonly, isRequired, label, labelEdge, labelStartWidth = '33%', parentComponentVariant, messages, userAssistanceDensity, role, children }, ref = null) => {
    ...
    const { groupProps, labelProps, topUserAssistanceProps, bottomUserAssistanceProps } = useCheckboxRadioField({
        ariaDescribedBy,
        baseId,
        isRequired,
        isReadonly,
    ...
    const [isFocusRingShown, focusRingProps] = useCollectionFocusRing((elem) => {
        return !!groupRef.current?.contains(elem);
    }, ['ArrowUp', 'ArrowRight', 'ArrowDown', 'ArrowLeft']);
    const content = (jsxs(Fragment, { children: [hasTopUserAssistance && (jsx("div", { id: `${baseId}-top-ua-container`, class: labelEdge === 'start' ? topUAContainerStyles.start : topUAContainerStyles.top, children: jsx(InlineUserAssistanceContainer, { ...topUserAssistanceProps, children: jsx(InlineHelp, { assistiveText: assistiveText, sourceLink: helpSourceLink, sourceText: helpSourceText }) }) })), jsx(Flex, { direction: direction, ...(direction === 'row' && { gap: xUnits(8) }), children: children }), hasBottomUserAssistance && !isReadonly && (jsx(InlineUserAssistance, { ...bottomUserAssistanceProps, requiredAlignment: "start", messages: messages, userAssistanceDensity: userAssistanceDensity === 'reflow' ? userAssistanceDensity : 'efficient', id: `${baseId}-bottom-ua-container` }))] }));
    // TODO: spacing under label when labelEdge === 'top' and hasTopUA.
    return (jsxs("div", { role: role, ref: groupRef, ...groupProps, ...focusRingProps, children: [labelEdge === 'none' && content, labelEdge !== 'none' && (jsx(LabelValueLayout, { parentComponentVariant: parentComponentVariant, hasTopUserAssistance: hasTopUserAssistance, label: jsx(Label, { ...labelProps, variant: labelEdge, userAssistanceDensity: "efficient", parentComponentVariant: parentComponentVariant, children: label }), labelEdge: labelEdge === 'inside' ? 'top' : labelEdge, labelStartWidth: labelStartWidth, children: jsx(CheckboxRadioContext.Provider, { value: { isFocusRingShown }, children: content }) }))] }));
});

export { CheckboxRadioField };

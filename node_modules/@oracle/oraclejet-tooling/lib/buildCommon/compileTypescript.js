    ...
    if (util.hasJsdocInstalled() && (isVComponent || vComponentsInPack !== 0)) {
      if (isVComponent && vComponentsInPack.length === 0) {
        ...
        vComponentsInPack.forEach((vcomponent) => {
          console.log(`Generating API docs for vcomponent ${vcomponent} in ${component}.`);
        });
      }
...
function _modifyConfigFileInStaging(srcPath, templatesFolder) {
  ...
  try {
    const pathToApiTemplatesInSrc = path.join(util.destPath(), 'src', 'apidoc_templates');
  ...
  compositeComponents.forEach((compositeComponent) => {
    ...
    if (componentJson.type === 'pack' || componentJson.type === CONSTANTS.PACK_TYPE.MONO_PACK) {
      ...
      vComponentsInMonoPack.forEach((vComponent) => {
        ...
        [
          pathToStubFile,
          pathToStubFileInTypesFolderRoot,
          pathToStubFileInComponentTypesFolder
        ].forEach((stubFile) => {
          if (fs.existsSync(stubFile)) {
            const fileContent = fs.readFileSync(stubFile, { encoding: 'utf-8' });
            const declaredProp = `'${compositeComponent}-${vComponent}': any`;
            if (fileContent.replace(/\s*:\s*/g, ': ').includes(declaredProp)) {
              fs.writeFileSync(
                stubFile,
                fileContent.replaceAll(/declare\s*global\s*{([\s\S]*)\}/gm, '')
              );
            }
          }
        });
